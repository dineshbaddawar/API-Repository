({
	/*doInit : function(cmp, event, helper) {
        debugger;
        var action=cmp.get('c.checkPaymentTermDetails');
        action.setParams({
            ...
            if(state == 'SUCCESS') {
                ...
                if(a.getReturnValue()!==null)
                {
                    var response=a.getReturnValue();
                         //cmp.set('v.siteURL','/apex/QuotePDF?id='+cmp.get('v.recordId'));
                }else{
                    var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        title : 'Warning',
                        message: 'Please select atleast one payment term to generate quote.',
                        duration:' 5000',
                        key: 'info_alt',
                        type: 'warning',
                        mode: 'pester'
